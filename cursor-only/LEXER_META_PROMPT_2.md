# Meta-Prompt: TypeScript Compiler Lexer (Iterative Build, DFA Transition Table)
...
**Core implementation style (required):** Build the lexer as a **strict table-driven DFA**. Token recognition and state evolution must come from transition-table lookup (`nextState = table[currentState][charClass]`), not from ad-hoc token `if/else` or `switch` decision trees.
...
### DFA Architecture Requirements
- Define an explicit lexer state model (e.g., `enum LexerState`)
- Define character classes (e.g., letter, digit, whitespace, slash, star, quote, symbol, EOF)
- Implement a transition table: `(state, charClass) -> nextState`
- Define accepting states and map them to token types
- Define explicit reject/error states and recovery behavior
- Apply **maximal munch** (longest valid lexeme) where applicable
- Perform keyword resolution after identifier recognition
- Use table lookup for **all transition decisions**, including comment/string/operator flows
- Keep side effects outside transition logic (emitting tokens, logging, warnings/errors happen after state resolution)
- Keep output behavior identical to expected format while using DFA internally

### Not Allowed (for token recognition logic)
- No large token-dispatch chains like `if (c === "{") ... else if (...) ...`
- No `switch`-based transition logic that replaces tables for DFA evolution
- No separate ad-hoc recognizers that bypass the table (except minimal I/O glue and reporting)
...
### **Step 1: Project Scaffolding and DFA Core Structure**
...
- Create a minimal `Lexer` class with:
  ...
  - DFA skeleton: state enum, char-classifier, transition tables, accepting-state map, error-state map
  - Skeleton for `lex()` that iterates using only table-driven state transitions
- Add a simple CLI or entry point that reads input
- **Deliverable**: Project runs, reads input, strict table-driven DFA scaffolding exists, reports nothing yet. Ready for token logic.

---

### **Step 2: Delimiters and Block Structure via DFA**
...
- Encode these symbols through DFA transitions/accepting states (no direct token `if` dispatch)
- **Test**: Input `{}$` -> matches `output1.txt`. Input `{{{{{{}}}}}}$` -> matches `output2.txt`
- **Deliverable**: Block delimiters and EOP work via transition table; output format matches examples.

---

### **Step 3: Comments in DFA Flow**
...
- Model comment scanning with a dedicated **table-driven** comment DFA (states + transition table + accept/reject states)
...
- Use identifier DFA recognition + keyword lookup table to assign final token type
...
- **Deliverable**: All keywords and types lex correctly via table-driven DFA paths.
...
- Implement `==`, `!=` (boolop)
- Implement `+` (intop)
- Ensure maximal munch in DFA/operator handling (`==` must beat `=`, etc.) through table transitions and accepting-state precedence
...
- Model number/string lexeme consumption with table-driven DFA states
...
- Ensure DFA error states/reporting are deterministic and easy to debug
...
- Confirm DFA tables/states are readable and maintainable
